...
def show_other_cars():
    ...
    cars = pd.read_csv("static/similar_car_result.csv")
    cars = cars.loc[cars['Car'] == car]

    categories = pd.read_csv("static/common_aspect_result.csv")
    categories = categories.loc[categories['Car'] == car]

    return render_template('compare_cars.html',
                            cars=cars.dropna(axis=1).values[0][1:],
                            categories=categories.dropna(axis=1).values[0][1:],
                            car=car)
